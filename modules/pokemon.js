...
export default class Pokemon {
  ...
  takeDamage(damage) {
    //absorbs some damage based on pokemon's defense stat
    this.health += -damage + Math.round((this.defense / 100) * damage);
    return;
  }

  calculateDamage(move) {
    //boosts attack based on pokemon's attack stat
    return this.attack + Math.round((move.power * this.attack) / 100);
  }

  getMove(string) {
    //Gets move object given the name of a move
  ...
  consumePotion() {
    //going to need to flesh this out
