...
export default class BattleUIController {
  ...
  configureMoveButtons(battleConductor) {
    for (let moveButton of this.moveButtons) {
      moveButton.addEventListener("click", () => {
        ...
        if (
          this.announcerBox.textContent.startsWith(
            `${battleConductor.user.pokemon.name} used`
          )
        ) {
  ...
  configureBattleProgression(battleConductor) {
    //can also update health of UI here depending on what exactly happened
    this.gameboyShell.addEventListener("click", () => {
      if (
        this.announcerBox.innerText == `What would you like Charizard to use?`
      ) {
        return;
      }
  ...
  progressBattle(battleConductor) {
    ...
    if (
      this.announcerBox.textContent.startsWith(
        `${battleConductor.cpu.pokemon.name} used`
      )
    ) {
  ...
  updateUserHealthBar(battleConductor) {
    ...
    let numPixelsUser = Math.ceil(
      userPercentage * this.userHealthContainer.clientWidth
    );
  ...
  updateEnemyHealthBar(battleConductor) {
    ...
    let numPixelsCpu = Math.ceil(
      cpuPercentage * this.enemyHealthContainer.clientWidth
    );
